...
;  set extra_leaving_energy 0
  ...
  ask fields [
    ...
    ifelse hab = "grass"
     [set hab_biomass (16.4 * initial_grass_length * area)  set gr_length (hab_biomass / area / 16.4) set color green] ;the initial grass value for all the fields is 2cm and change into biomass (g/field) as described in the ODD.
     [ifelse hab = "plough"
       [set hab_biomass 0 set color gray]
       [ifelse hab = "potato"
         [set hab_biomass 0 set color brown]
         [ifelse hab = "stubble"
           [set hab_biomass (initial_grain_density_stubble * area) set grain_dens initial_grain_density_stubble set color yellow] ; average grain density as measured by me and Bart in 2013
           [ifelse hab = "grain"
             [set hab_biomass (initial_grain_density_grain * area) set grain_dens initial_grain_density_grain set color orange] ; average grain density as measured by me and Bart in 2013
             [set color 127]
           ]
         ]
       ]
...
;we assume energy expenditure equal to average energy expenditure of one hour
  ...
  let CloseGrassFields fields with [hab = "grass" and distance myself < (5 / patch_size)]
  let CloseStubbleFields fields with [hab = "stubble" and distance myself < (5 / patch_size)]
  let CloseGrainFields fields with [hab = "grain" and distance myself < (5 / patch_size)]

  if any? CloseGrassFields
  [
    let av_gr_length (mean [gr_length] of CloseGrassFields)
  ...
  if any? CloseStubbleFields
  [
    let av_stubble (mean [grain_dens] of CloseStubbleFields)
  ...
  if any? CloseGrainFields
  [
    let av_grain (mean [grain_dens] of CloseGrainFields)
    let ir_grain 3600  * ((a_grain * av_grain) / (1 + a_grain * h_grain * av_grain))
    set mei_grain ((ir_grain * gf_grain) - (dr_grain * m_grain * gd_grain))
  ]

  if any? fields with [hab = "potato" and distance myself < (5 / patch_size)]
  [
    set mei_pot mei_potato
  ]

    if any? fields with [hab = "plough" and distance myself < (5 / patch_size)]
  [
    set mei_ploughed 0
  ]


   ifelse decision_making_rule = "max energy"
    [set gamma (mean (list mei_grass mei_stubble mei_grain mei_pot) - av_ee)]
    [set gamma (mean (list mei_grass mei_stubble mei_ploughed mei_grain mei_pot) - av_ee)]
...
;because it is all in one bracket, everything is done by one agent, then by another etc. it matters in depletion process
      ...
      if ticks = 792 ; corresponds to 08.05
      [
        ask roosts
        [
        ...
        output-number-of-geese-on-roost
      ]
...
to-report with-max-first [lists]
  ...
  let winning? [ ?1 -> first ?1 = highest-first ]
    ...
    [
      ...
      [
        ...
        ifelse any? field1km
          ...
          [
            move-to one-of fields with [distance myself < (big_radius / patch_size)]
            ;show "dist, no cum, no 500m I moved to a random field within 5km becuase there was no field within 1km"
            stop
          ]
    ...
    if decision_making_rule = "random" or decision_making_rule = "asocial learning"
      [
        ...
        ifelse any? field1km
          ...
          [
            move-to one-of fields with [distance myself < (big_radius / patch_size)]
            ;show "I left this field to a random in 1km because of GU becuase there was no field within 1km"
          ]
...
to field-with_highest_nei_within_1km
  ...
  let CloseGrassFields fields with [hab = "grass" and distance myself < (small_radius / patch_size)]
  let CloseStubbleFields fields with [hab = "stubble" and distance myself < (small_radius / patch_size)]
  let CloseGrainFields fields with [hab = "grain" and distance myself < (small_radius / patch_size)]
  let ClosePotatoFields fields with [hab = "potato" and distance myself < (small_radius / patch_size)]

  ifelse any? CloseGrassFields
  [
    let field_with_max_gr_length max-one-of CloseGrassFields [gr_length]
    let max_gr_length [gr_length] of field_with_max_gr_length
    ;set id_field_with_max_grass ([who] of field_with_max_gr_length)  ;; You can delete this...
  ...
  ifelse any? CloseStubbleFields
  [
    let field_with_max_stubble max-one-of CloseStubbleFields [grain_dens]
    let max_stubble [grain_dens] of field_with_max_stubble
    ;set id_field_with_max_stubble ([who] of field_with_max_stubble)
  ...
  ifelse any? CloseGrainFields
  [
    let field_with_max_grain max-one-of CloseGrainFields [grain_dens]
    let max_grain [grain_dens] of field_with_max_grain
    ;set id_field_with_max_grain ([who] of field_with_max_grain)
  ...
  ifelse any? ClosePotatoFields
  [
    let potential_potato one-of ClosePotatoFields
    set mei_pot mei_potato
  ...
  ifelse max_mei = mei_grass
    [
      move-to max-one-of CloseGrassFields [gr_length]
      ;show "Im on grass with highest mei"
    ]
    [
      ifelse max_mei = mei_stubble
      [
        move-to max-one-of CloseStubbleFields [grain_dens]
        ;show "Im on stubble with highest mei"
      ]
      [
        ifelse max_mei = mei_grain
        [
          move-to max-one-of CloseGrainFields [grain_dens]
          ;show "Im on grain with highest mei"
        ]
        [
          move-to one-of ClosePotatoFields
...
to field-with_highest_nei_within_5km
  ...
  let CloseGrassFields fields with [hab = "grass" and distance myself < (big_radius / patch_size)]
  let CloseStubbleFields fields with [hab = "stubble" and distance myself < (big_radius / patch_size)]
  let CloseGrainFields fields with [hab = "grain" and distance myself < (big_radius / patch_size)]
  let ClosePotatoFields fields with [hab = "potato" and distance myself < (big_radius / patch_size)]

  ifelse any? CloseGrassFields
  [
    let field_with_max_gr_length max-one-of CloseGrassFields [gr_length]
    let max_gr_length [gr_length] of field_with_max_gr_length
    ;set id_field_with_max_grass ([who] of field_with_max_gr_length)
    let ir_grass 3600 * (1 / ((1 + b2 * max_gr_length / 100) / (b1 * max_gr_length / 100)*(tc + b3 * max_gr_length / 100) + 1 / R_max))
    set mei_grass ((ir_grass * gf_grass) - (dr_grass *  m_grass * gd_grass))
  ]
  [
...
;if they are already on a roost they stay there until day comes
  ...
  ifelse any? roosts-here
    ...
    [
      ...
      ifelse decision_making_rule = "random"
      [
...
;;;;;;;;;;;;;;;;;;;;; WHAT HAPPENDS TO A GOOSE AGENT IF IS ON A FIELD ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
; if geese happen to forage on habitat were habitat biomass is close to 0, I wrote a code to be sure that habitat biomass cannot go below 0
...
[
 ...
 if [hab] of closest_field = "grass" and [gr_length] of closest_field > 0
   [
       ...
       let hab_biomass_closest_field [hab_biomass] of closest_field
       let remaining_biomass  hab_biomass_closest_field - (n_geese_in_super_goose * ir)
       if remaining_biomass < 0 [set ir hab_biomass_closest_field / n_geese_in_super_goose]

       set step_eg ((ir * gf_grass) - (dr_grass * m_grass * gd_grass))

       ifelse disturbed? = true
        [set step_eg (step_eg * (1 - eg_reduction_dist))]       ; set step_eg (step_eg - (eg_reduction_dist * step_eg))
        [set step_eg step_eg]
 ...
 if [hab] of closest_field = "grain" and [grain_dens] of closest_field > 0
   [
       ...
       ifelse disturbed? = true
        [set step_eg (step_eg * (1 - eg_reduction_dist))]
        [set step_eg step_eg]
       if step_eg < 0 [set step_eg 0]
 ...
 if [hab] of closest_field = "stubble" and [grain_dens] of closest_field > 0
   [
       set pr_feed ((random-float 0.241) + 0.54)
...
ifelse ticks_since_morning > 1
  [
   ...
   ifelse disturbed? = true
   [
     set cum_time_feeding (cum_time_feeding + pr_feed * (1 - eg_reduction_dist))           ; set cum_time_feeding (cum_time_feeding + (pr_feed - pr_feed * eg_reduction_dist))
     set cum_tot_time_feeding (cum_tot_time_feeding + pr_feed * (1 - eg_reduction_dist))   ; set cum_tot_time_feeding (cum_tot_time_feeding + (pr_feed - pr_feed * eg_reduction_dist))
   ]
   [
...
<experiments>
  ...
  <experiment name="dmr1-pattern5-expindex277" repetitions="3" runMetricsEveryStep="true">
    <setup>setups</setup>
    <go>go</go>
    <timeLimit steps="1057"/>
    <exitCondition>count geese = 0</exitCondition>
    <metric>gl_max_num_geese_per_roost</metric>
    <enumeratedValueSet variable="steps_on_500m_roost">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="temp_disturbance">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="alpha">
      <value value="0.05"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial_grass_length">
      <value value="3"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="roost_dist_radius">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="n_geese_in_super_goose">
      <value value="20"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="prob-of-disturbance-large-fields">
      <value value="0.2"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="prob-of-disturbance-small-fields">
      <value value="0.3"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="big_radius">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="expindex">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="extra_initial_energy">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max_steps-to_stay_on_roost">
      <value value="3"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="grain_mean_stubble">
      <value value="22"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="grain_sd_stubble">
      <value value="22"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial_grain_density_grain">
      <value value="17"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="cum_time_threshold">
      <value value="3"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="ee_fly">
      <value value="416.35"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="extra_leaving_energy">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="av_ee">
      <value value="54.28"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="small_radius">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="starvation_energy_stores">
      <value value="9620"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="decision_making_rule">
      <value value="&quot;random&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="field_area_threshold">
      <value value="60000"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="dmr1-pattern5-expindex347" repetitions="3" runMetricsEveryStep="true">
    <setup>setups</setup>
    <go>go</go>
    <timeLimit steps="1057"/>
    <exitCondition>count geese = 0</exitCondition>
  ...
  <experiment name="dmr1-pattern5-expindex417" repetitions="3" runMetricsEveryStep="true">
    <setup>setups</setup>
    <go>go</go>
    <timeLimit steps="1057"/>
    <exitCondition>count geese = 0</exitCondition>
  ...
  <experiment name="dmr3-pattern5-expindex277" repetitions="3" runMetricsEveryStep="true">
    <setup>setups</setup>
    <go>go</go>
    <timeLimit steps="1057"/>
    <exitCondition>count geese = 0</exitCondition>
  ...
  <experiment name="dmr3-pattern5-expindex347" repetitions="3" runMetricsEveryStep="true">
    <setup>setups</setup>
    <go>go</go>
    <timeLimit steps="1057"/>
    <exitCondition>count geese = 0</exitCondition>
  ...
  <experiment name="dmr3-pattern5-expindex417" repetitions="3" runMetricsEveryStep="true">
    <setup>setups</setup>
    <go>go</go>
    <timeLimit steps="1057"/>
    <exitCondition>count geese = 0</exitCondition>
  ...
  <experiment name="dmr4-pattern5-expindex277" repetitions="3" runMetricsEveryStep="true">
    <setup>setups</setup>
    <go>go</go>
    <timeLimit steps="1057"/>
    <exitCondition>count geese = 0</exitCondition>
  ...
  <experiment name="dmr4-pattern5-expindex347" repetitions="3" runMetricsEveryStep="true">
    <setup>setups</setup>
    <go>go</go>
    <timeLimit steps="1057"/>
    <exitCondition>count geese = 0</exitCondition>
  ...
  <experiment name="dmr4-pattern5-expindex417" repetitions="3" runMetricsEveryStep="true">
    <setup>setups</setup>
    <go>go</go>
    <timeLimit steps="1057"/>
    <exitCondition>count geese = 0</exitCondition>
  ...
  <experiment name="example-Daniel" repetitions="100" runMetricsEveryStep="true">
    <setup>setups</setup>
    <go>go</go>
    <timeLimit steps="1057"/>
    <exitCondition>count geese = 0</exitCondition>
  ...
  <experiment name="dmr5-pattern5-expindex347" repetitions="3" runMetricsEveryStep="true">
    <setup>setups</setup>
    <go>go</go>
    <timeLimit steps="1057"/>
    <exitCondition>count geese = 0</exitCondition>
  ...
  <experiment name="dmr5-pattern5-expindex417" repetitions="3" runMetricsEveryStep="true">
    <setup>setups</setup>
    <go>go</go>
    <timeLimit steps="1057"/>
    <exitCondition>count geese = 0</exitCondition>
  ...
  <experiment name="example-Daniel2" repetitions="100" runMetricsEveryStep="true">
    <setup>setups</setup>
    <go>go</go>
    <timeLimit steps="1057"/>
    <exitCondition>count geese = 0</exitCondition>
